...
if [ -f ".env" ]; then
  Z_MODEL=$(grep -E '^Z_AI_MODEL=' .env 2>/dev/null | cut -d'=' -f2- | tr -d '"' | tr -d "'")
  Z_KEY_LINE=$(grep -E '^Z_AI_API_KEY=' .env 2>/dev/null | cut -d'=' -f2- | tr -d '"' | tr -d "'")

  if [ -n "$Z_MODEL" ]; then
    CONTEXT="$CONTEXT | Z.ai model: $Z_MODEL"
  else
    CONTEXT="$CONTEXT | Z.ai model: NOT SET"
  fi

  if [ -n "$Z_KEY_LINE" ] && [ "$Z_KEY_LINE" != "your_api_key_here" ]; then
    CONTEXT="$CONTEXT | Z_AI_API_KEY: set"
  else
    CONTEXT="$CONTEXT | Z_AI_API_KEY: NOT SET — add to .env"
  fi
fi
