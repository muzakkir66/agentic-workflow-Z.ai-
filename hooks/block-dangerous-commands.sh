...
# ──────────────────────────────────────────────
# Z.ai credential exposure
# ──────────────────────────────────────────────

# Block printing the full .env file to stdout (exposes Z_AI_API_KEY)
if echo "$COMMAND" | grep -qE '(cat|less|more|type)[[:space:]]+.*\.env([[:space:]]|$)'; then
  deny "Blocked: printing .env would expose your Z_AI_API_KEY. Read specific values with grep instead: grep Z_AI_MODEL .env"
fi

# Block echoing the raw API key variable to stdout
if echo "$COMMAND" | grep -qE 'echo[[:space:]].*\$Z_AI_API_KEY'; then
  deny "Blocked: echoing \$Z_AI_API_KEY would expose your API key in the terminal and logs."
fi
