...
# Z.ai API key hardcoded (catches Z_AI_API_KEY = "actual_key" in any file)
if echo "$CONTENT" | grep -qiE 'Z_AI_API_KEY[[:space:]]*[=:][[:space:]]*["\x27]?[A-Za-z0-9._\-]{16,}' && \
   ! echo "$CONTENT" | grep -qiE 'Z_AI_API_KEY[[:space:]]*[=:][[:space:]]*["\x27]?(process\.env|\$\{|your_api_key_here)'; then
  MATCHES="$MATCHES Z.ai API key (Z_AI_API_KEY);"
fi
