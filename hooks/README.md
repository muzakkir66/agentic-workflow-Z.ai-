...
- **Z.ai credentials**: `cat .env` (exposes `Z_AI_API_KEY`), `echo $Z_AI_API_KEY`
- **System**: `chmod 777`, piping `curl`/`wget` to `bash`/`sh`, `mkfs`, `dd if=`, writes to `/dev/`
- **Publishing**: `npm publish` (and `cargo publish`, `gem push`, `twine upload`)

### scan-secrets.sh
**Event**: PreToolUse (Edit|Write)

Scans file content for accidental secrets before writing. Uses `ask` (not hard deny) so test fixtures can be overridden. Detects:
- **Z.ai API key**: `Z_AI_API_KEY` assigned a literal value (not a `process.env` reference)
...
Injects dynamic project context at session start: current branch (or detached HEAD warning), last commit, uncommitted changes count, staged changes indicator, stash count, and Z.ai environment status (active model from `.env`, whether `Z_AI_API_KEY` is set).
